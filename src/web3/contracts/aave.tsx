...
const CONTRACT_AAVE_ADDR = String(process.env.REACT_APP_CONTRACT_BOND_ADDR).toLowerCase();

const Contract = new Web3Contract(BOND_ABI as Web3ContractAbiItem[], CONTRACT_AAVE_ADDR, 'BOND');

export const AAVETokenMeta: TokenMeta = {
  icon: <Icon key="aave" name="png/aave" width={40} height={40} />,
  name: 'AAVE',
  address: CONTRACT_AAVE_ADDR,
  decimals: 18,
};

type AAVEContractData = {
  balance?: BigNumber;
...
export type AAVEContract = AAVEContractData & {
  contract: Web3Contract;
  reload(): void;
  approveSend(address: string, value: BigNumber): Promise<any>;
};

const InitialData: AAVEContractData = {
  balance: undefined,
  allowance: undefined,
  barnAllowance: undefined,
};

export function useAAVEContract(): AAVEContract {
  const [reload] = useReload();
  const wallet = useWallet();

  const [data, setData] = React.useState<AAVEContractData>(InitialData);
  ...
  React.useEffect(() => {
    (async () => {
      ...
      [totalSupply] = await Contract.batch([
        {
          method: 'totalSupply',
          transform: (value: string) => getHumanValue(new BigNumber(value), AAVETokenMeta.decimals),
        },
      ]);
...
export class AaveContract extends Erc20Contract {
  constructor() {
    super([], CONTRACT_AAVE_ADDR);
  }
}
